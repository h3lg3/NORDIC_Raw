...
if ( ARG.save_gfactor_map==2 )  | ( ARG.save_gfactor_map==1 )
    ...
    niftiwrite((g_IMG),[ARG.DIROUT 'gfactor_' ARG.fn_out(1:end) '.nii'])
...
if  ~isfield(ARG,'use_magn_for_gfactor')  %% WOULD THIS BE THE ISSUE  & replaced by |
...
if isfield(ARG,'save_residual_matlab')
    if ARG.save_residual_matlab==1;
        ...
        save(fullfile(ARG.DIROUT, ['RESIDUAL' ARG.fn_out '.mat']),'Residual','-v7.3')
    end
end
...
if isfield(ARG,'save_add_info')
    if  ARG.save_add_info==1
        ...
        save(fullfile(ARG.DIROUT, [ARG.fn_out '.mat']),'ARG2','ARG','-v7.3')
    end
end
